...
- 支持 CAD 导入与三维可视化
- 支持 CAD 表面点位标注（点 + 法线），拍摄方向自动取 `-normal`
- 所有技术参数（相机配置、规划选项）使用系统默认值，不在 UI 中暴露
- 支持规划结果预览与任务执行控制（启动/暂停/继续/停止）
...
- `Edit / Task`（右侧栏 2 Tab）
  - Tab 1 — 标注：选中点位只读信息（ID/位置/法向）+ 删除按钮 + 点位计数
  - Tab 2 — 任务：任务名 + 提交并规划 + 规划结果 + 执行控制（启动/暂停/继续/停止）+ 进度 + 系统状态（AGV/机械臂/联锁）+ 事件日志
  - 注意：相机参数（ID/焦距/FOV/倾角）与规划参数（候选半径/偏航步长/IK求解器/碰撞检测/TSP）均使用系统默认值，不在 UI 中暴露
- `Status & Log`
  - 当前步骤、错误信息
  - 接口调用日志
  - 机器人状态卡（AGV / 机械臂 / 联锁）
...
3. 点击 `提交并规划`（系统自动使用默认拍摄配置和规划参数）
4. 展示路径预览与统计（点位数、里程、规划耗时）
5. 确认后启动任务
...
- `导入 CAD` -> `UploadCad`
- `提交并规划` -> `SetInspectionTargets` + `PlanInspection`（拍摄配置与规划参数使用系统默认值）
- `断点恢复/重连加载规划` -> `GetPlan`
...
- 可导入 STL/OBJ 并显示
- 可完成点位标注并保存点位数据（点 + 法线，拍摄方向自动取 `-normal`）
- 可一键提交并规划（系统自动填充拍摄配置与规划参数）
